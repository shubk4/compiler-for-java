...
void add_op(quad* instr){
    if(is_integer(instr->arg1) && is_integer(instr->arg2)){
        int val = (stoi(instr->arg1) + stoi(instr->arg2));
        string str = get_mem_location(&instr->res, &empty_var, stoi(instr->idx), 0);
        code_file << "\tmov " << str <<", " << "dword "<< val << "\n";
    }
    else{
        string reg1 = getReg(&instr->arg1, &instr->res, &instr->arg2, stoi(instr->idx));
        string mem2 = get_mem_location(&instr->arg2, &instr->arg1, stoi(instr->idx), 0);
        // if(instr->arg1.second->is_derefer){
        //     string str = get_mem_location(&instr->arg1, &instr->arg2, stoi(instr->idx), -1);
        ...
        //     string temp_reg = getTemporaryReg(&instr->arg2, stoi(instr->idx));
...
//         free_reg("eax");
//         free_reg("edx");
//         string str = get_mem_location(&instr->arg1, &instr->arg2, stoi(instr->idx), 0);
...
//         string reg2 = getReg(&instr->arg2, &instr->res, &empty_var, stoi(instr->idx));
//         if(instr->arg2.second->is_derefer){ 
//             string str = get_mem_location(&instr->arg2, &instr->arg1, stoi(instr->idx), -1);
...
//         string reg = get_mem_location(&instr->res, &empty_var, stoi(instr->idx), 1);
//         if(a && b){
...
//         l1 = get_label();
//         l2 = get_label();
//         string reg = getReg(&instr->arg1, &instr->res, &instr->arg2, stoi(instr->idx));    
//         if(instr->arg1.second->is_derefer){
//             string str = get_mem_location(&instr->arg1, &instr->arg2, stoi(instr->idx), -1);
...
//         string mem2 = getReg(&instr->arg2, &empty_var, &instr->arg1, stoi(instr->idx));
...
//         string mem = get_mem_location(&instr->res, &empty_var, stoi(instr->idx), 1);
//         code_file << "\tmov " << mem <<", " << "dword "<< val << "\n";
//         return;
//     }

//     string reg = getReg(&instr->arg1, &empty_var, &instr->arg2, stoi(instr->idx));
...
//                 free_reg(it.first);
//             }
//             string reg1 = getTemporaryReg(&empty_var, stoi(instr->idx));
            
//             exclude_this.insert(reg1);
            
//             string reg2 = getTemporaryReg(&empty_var, stoi(instr->idx));
//             exclude_this.clear();
...
//         }
//         else{
//             string mem = get_mem_location(&instr->arg1, &empty_var, stoi(instr->idx), 1);
...
//     end_basic_block();
//     int id = stoi(instr->idx);
//     if(instr->arg1.first == "IF"){
//         string reg = get_mem_location(&instr->arg2, &instr->arg1, id, 1);
//         if(is_integer(instr->arg2.first)) reg = getReg(&instr->arg2, &empty_var, &empty_var, stoi(instr->idx));
//         code_file << "\tcmp " << reg <<", " << "dword 0"<<"\n";
...
//                 string reg = getReg(&params.top(), &empty_var, &empty_var, stoi(instr->idx));
...
//             string mem;
//             mem = get_mem_location(&params.top(), &empty_var, stoi(instr->idx), 1);
//             code_file<<"\tpush dword "<<mem<<"\n";
//         }
//         else if(params.top().second->isArray){
//             string str = get_mem_location(&params.top(), &empty_var, stoi(instr->idx), 1);
//             code_file<<"\tlea eax, "<< str <<"\n";
//             code_file<<"\tpush eax\n";
//         }
//         else{
//             string mem = get_mem_location(&params.top(), &empty_var, stoi(instr->idx), 1);
...
void assign_op(quad* instr){
    ...
    // }
    // x = 1
    if(is_integer(instr->arg1)){
        string mem = get_mem_location(&instr->res, &instr->arg1, stoi(instr->idx), 1);
        // if(reg_desc.find(mem) != reg_desc.end()){
        //     free_reg(mem);
        //     update_reg_desc(mem, &instr->res);
        // }
        code_file << "\tmov "<< mem << ", dword "<< instr->arg1 <<endl;
    }
    ...
    //     string reg = getTemporaryReg(&empty_var, stoi(instr->idx));
    ...
    // x = y
        ...
        string reg = getReg(&instr->arg1, &instr->res, &empty_var, stoi(instr->idx));
        ...
        //     string tem = get_mem_location(&instr->res,&empty_var,stoi(instr->idx),-1);
        ...
        //     string str = get_mem_location(&instr->res, &instr->arg1, stoi(instr->idx), 0);
...
//         if(instr->arg1.second->is_derefer){
//             string reg = getReg(&instr->arg1, &instr->res, &instr->arg2, stoi(instr->idx));
//             string reg1 = getTemporaryReg(&instr->arg1, stoi(instr->idx));
...
//             code_file<<"\t"<<instruction<<" "<<reg<<endl;
//             string str = get_mem_location(&instr->arg1, &empty_var,stoi(instr->idx),-1);
...
//         string reg = getReg(&instr->res, &empty_var, &instr->arg1, stoi(instr->idx));
//         string mem = get_mem_location(&instr->arg1, &instr->arg2, stoi(instr->idx), 2);

//         if(instr->arg1.second->is_derefer){
//             code_file<<"\tmov "<<reg<<", "<<mem<<"\n";
//         }
//         else{
//             if(reg_desc.find(mem) != reg_desc.end()) mem =  get_mem_location(&instr->arg1, &instr->arg2, stoi(instr->idx), -1);
...
//         string reg = getReg(&instr->res, &empty_var, &instr->arg2, stoi(instr->idx));
//         string mem, str;
...
//                 instr->arg1.second->addr_descriptor.reg = "";
//                 str = get_mem_location(&instr->arg1, &instr->arg2, stoi(instr->idx), 0);
//                 code_file<<"\tmov "<< str <<", "<<temp_reg<<"\n"; 
//             }
//             mem = get_mem_location(&instr->arg1, &instr->arg2, stoi(instr->idx), 0);
//             code_file<<"\tlea "<<reg<<", "<<mem<<"\n";
//         }
//         else {
//             mem = get_mem_location(&instr->arg1, &instr->arg2, stoi(instr->idx), 0);
...
//         string reg1 = getReg(&instr->arg2, &empty_var, &instr->arg1, stoi(instr->idx));
//         if(instr->arg2.second->is_derefer){
//             string str = get_mem_location(&instr->arg1, &instr->arg2, stoi(instr->idx), -1);
//             code_file<<"\tmov "<<str <<", "<< reg1<<endl;
//             code_file<<"\tmov "<<reg1<<", [ "<<reg1<<" ]\n";
...
//     string mem = get_mem_location(&instr->res, &empty_var, stoi(instr->idx), 0);
...
void genCode(){
    ...
    for (auto it=leaders.begin(); it != leaders.end(); it++){
        ...
        for(int idx=start; idx < end; idx++){
            quad instr = ir[idx];
            if(instr.arg1 != "") instr.arg1 = char_to_int(instr.arg1);
            if(instr.arg2 != "") instr.arg2 = char_to_int(instr.arg2);
            
            // if(debug_mode) code_file<<"\t\t\t\t\t\t\t\t\t\t;"<<instr.arg1<<" "<<instr.op<<" "<<instr.arg2<<" "<<instr.res<<"\n";
            
            // if(instr.op.substr(0, 5) == "FUNC_" && instr.op[(instr.op.size() - 3)] == 't'){
            //     in_func = 1;
            //     gen_func_label(&instr);   //not done!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            // }



            // // else if(instr.op.substr(0,2) == "++"  
            // //         ||instr.op.substr(0,2) == "--" 
            // //         ||instr.op == "!" 
            // //         ||instr.op == "~" 
            // //         ||instr.op == "unary-" 
            //         ||instr.op == "unary+") unary_op(&instr);
            // else if(instr.op == "unary&" || instr.op == "unary*") pointer_op(&instr);




            else if(instr.op[0] == '+')    add_op(&instr);
            else if(instr.op == "=")   assign_op(&instr);
            else if(instr.op.substr(0, 5) == "FUNC_" && instr.op[(instr.op.size() - 3)] == 'd'){
                // end_basic_block();
                code_file << "\txor eax, eax\n";
                code_file << "\tleave\n";
                code_file << "\tret\n";
            ...
            else if(instr.op[0] == '-') sub_op(&instr);
            // else if(instr.op[0] == '*') mul_op(&instr);
            // else if(instr.op[0] == '/') div_op(&instr);
            // else if(instr.op[0] == '%') mod_op(&instr);
            // else if(instr.op == "RETURN"){
            //     return_op(&instr);
            // }
            // else if(instr.op == "param") params.push(instr.arg1);
            // else if(instr.op == "CALL") call_func(&instr);
            // else if(instr.op == "=="  
            //         ||instr.op == "<" 
            //         ||instr.op == "<=" 
            //         ||instr.op == ">" 
            //         ||instr.op == ">=" 
            //         ||instr.op == "!=" ) comparison_op(&instr); 

            // else if(instr.op == "&&") logic_and(&instr);
            // else if(instr.op == "||") logic_or(&instr);
            // else if(instr.op.substr(0,2) == "<<") lshift_op(&instr);
            // else if(instr.op.substr(0,2) == ">>") rshift_op(&instr);
            // else if(instr.op[0] == '^' ||  instr.op[0] == '&' || instr.op[0] == '|') bitwise_op(&instr);
            // else if(instr.op == "GOTO") {    
            //     goto_op(&instr);
            // }
            // else if(instr.op == "PTR_OP") ptr_op(&instr);
            // else if(instr.op == "member_access") member_access(&instr);
            // else if(instr.op == "[ ]") array_op(&instr);
            // else if(instr.op == "SIZEOF") sizeof_op(&instr);
        }
        // end_basic_block();
    }
...
// void update_reg_desc(string reg, string* sym){
...
string get_mem_location(string * sym, string* sym2, int idx, int flag){
    ...
    return "mem";
    // if(is_integer(*sym)){
    //     if(flag) return string("dword " + *sym);
    //     else return *sym;
    // }
    // vector<string> symEntry = findEntry(*sym);

    // if(symEntry[8].reg != "" && flag!=-1){
    ...
    // //Symbol in stack
    // if(symEntry[9] == 0){
    //     int offset = symEntry[10].offset;
    //     int size = symEntry[10].size;
    //     string str;
    //     symEntry[8].stack = true;
    ...
    // symEntry[8].stack = false;
    // symEntry[8].heap = true;
    // return string("[ " + to_string(symEntry[9]) + " ]");
}
...
string getReg(string* sym, string* result, string* sym2, int idx){
    ...
    // if(sym[0] == '\"'){
    //     stringlabels[string_counter] = sym;
...
//             if(ir[i].arg1.first != "" && ir[i].arg1.first[0] == '#' && ir[i].arg1.second && ir[i].arg1.second->next_use == -1){
//                 ir[i].arg1.second->next_use = i;
//             } 
//             if(ir[i].arg2.first != "" && ir[i].arg2.first[0] == '#' && ir[i].arg2.second && ir[i].arg2.second->next_use == -1){
//                 ir[i].arg2.second->next_use = i;
//             }
//             if(ir[i].res.first != "" && ir[i].res.first[0] == '#' && ir[i].res.second && ir[i].res.second->next_use == -1){
//                 ir[i].res.second->next_use = i;
//             } 
//         }
//     }
// }

 // Finds Basic block in 3AC ir.
...
void findBasicBlocks(){
    for (int i=0;i< (int)ir.size(); i++){
        ...
        if(ir[i].op.substr(0, 5) == "FUNC_"){
                if(ir[i].op[ir[i].op.length()-3] == 't') leaders.insert(make_pair(i, get_label()));
                else if(i+1 != ir.size()) leaders.insert(make_pair(i+1, get_label()));
        }
        else if(ir[i].op == "GOTO"){
            leaders.insert(make_pair(stoi(ir[i].idx), get_label()));
            leaders.insert(make_pair(i+1, get_label()));
        }   
    }
    leaders.insert(make_pair(ir.size(), get_label()));
}
...
vector<int> findDeadCode(){
    ...
    for(auto it = leaders.begin(); it!=leaders.end(); ++it){
        ...
        if(ir[last].op == "GOTO"){
            adj_list[id].push_back(get_leader[stoi(ir[last].idx)]);
            if(ir[last].arg1 == "IF") adj_list[id].push_back(get_leader[last+1]);
        }
        else{
...
//     if(cnt>200) return j;
//     if((ir[j].op.first == "GOTO" && ir[j].arg1.first != "IF") && !(j>0 && ir[j-1].op.first == "GOTO" && ir[j-1].arg1.first == "IF")){
//         return (ir[j].idx = findDest(ir[j].idx, cnt+1));
//     }
//     return j;
// }
...
//     for(int i=0; i<(int)ir.size(); ++i){
//         if((ir[i].op.first == "GOTO" && ir[i].arg1.first != "IF") && !(i>0 && ir[i-1].op.first == "GOTO" && ir[i-1].arg1.first == "IF")){
//             ir[i].idx = findDest(ir[i].idx, 0);
//         }
//     }
// }
